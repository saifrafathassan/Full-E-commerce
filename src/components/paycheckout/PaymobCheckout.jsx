...
const paymobPaymentId = import.meta.env.VITE_PAYMOB_PAYMENT_ID;
...
export default function PaymobCheckout({ name, address, pincode, phoneNumber, setName, setAddress, setPincode, setPhoneNumber, totalAmount, cartItems }) {
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      window.location.href = `https://accept.paymob.com/api/acceptance/iframes/861601?payment_token=${paymentKey}`;
  
      toast.success("Payment initiated successfully. Please complete your payment.");
      
    } catch (error) {
      console.error('Error during Paymob payment:', error);
